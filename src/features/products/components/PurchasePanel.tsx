          ...
          <div className="grid grid-cols-3 gap-3">
            {specOptions.map((opt, i) => (
              <button
                ...
                className={`rounded-xl p-3 text-left flex flex-col justify-between transition-colors ${
                  activeSpec === i
                    ? 'border-2 border-primary bg-emerald-50/20 text-primary shadow-2xs'
              ...
              >
                ...
                <span className="text-xs font-bold text-primary mt-2">{formatVnd(opt.price)}</span>
              </button>
            ))}
          </div>
        </div>
