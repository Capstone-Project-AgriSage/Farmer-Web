...
function pickTheme(label: string) {
  ...
  if (
    text.includes("vườn") ||
    text.includes("sầu riêng") ||
    text.includes("nông") ||
    text.includes("canh tác") ||
    text.includes("năng suất")
  ) {
    return {
      title: "Nông nghiệp",
      icon: "field",
...
function applyLocalImage(img: HTMLImageElement) {
  ...
  if (
    !externalImagePattern.test(currentSrc) ||
  ...
  img.src = getFallbackImage(
    img.alt || img.getAttribute("aria-label") || "AgriSage",
  )
}
...
export function installImageFallbacks() {
  const applyAll = (root: ParentNode = document) => {
    root
      ...
      .forEach((img) => applyLocalImage(img as HTMLImageElement))
  }
