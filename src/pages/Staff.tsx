...
import type { StaffSection } from "./staff/StaffTypes"
...
const staffBySlug = Object.fromEntries(
  staffMenu.map((item) => [item.slug, item.id]),
) as Record<string, StaffSection>
...
function Badge({
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-[10px] font-black ${toneClass}`}
    >
      {label}
    </span>
  )
  ...
  return (
    <div className="min-h-screen bg-surface-subtle text-text-primary">
      <aside className="fixed inset-y-0 left-0 hidden w-64 border-r border-border-subtle bg-white lg:block">
        <Link
          to="/"
          className="flex h-20 items-center gap-3 border-b border-border-subtle px-5"
        >
          ...
          <div>
            ...
            <div className="text-[11px] font-black uppercase leading-tight tracking-wide text-text-muted">
              Nhân viên quầy (POS) & bán hàng
            </div>
          </div>
        </Link>
