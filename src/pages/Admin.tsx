...
const stats = [
  ...
  },
  {
    ...
    value: "346.250.000 đ",
    sub: "VietQR 58% - Tiền mặt 42%",
    icon: "account_balance",
    accent: "text-text-primary",
    progress: "76%",
  },
  {
    label: "Gối đầu vụ mùa",
    value: "136.400.000 đ",
    sub: "Tỷ trọng công nợ 28.3%",
    icon: "cycle",
...
function StatCard({
  ...
  return (
    <div className="rounded-xl border border-border-subtle bg-white p-4 shadow-sm transition-all hover:border-primary/35 hover:shadow-card">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="text-[11px] font-black uppercase tracking-wide text-text-muted">
          {label}
        </div>
        <span className="material-symbols-outlined rounded-lg bg-primary-light p-2 text-[18px] text-primary">
          {icon}
        </span>
      </div>
      <div className={`text-[22px] font-black leading-tight ${accent}`}>
        {value}
      </div>
      <div className="mt-4 text-xs font-bold text-primary">{sub}</div>
          ...
          <section className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-black text-text-primary">
                  Hàng đợi chẩn đoán cần kỹ sư xác nhận
                </h2>
                <p className="mt-1 text-xs text-text-muted">
                  Sắp xếp theo mức độ rủi ro mùa vụ, thời gian gửi ảnh và số lần
                  tái phát.
                </p>
              </div>
            ...
            <div className="space-y-3">
              {[
                ...
                (
                  [code, title, farmer, crop, confidence, status, time],
                  index,
                ) => (
                  <article
                    key={code}
                    className="grid gap-4 rounded-xl border border-border-subtle bg-surface-subtle p-4 md:grid-cols-[88px_1fr_auto] md:items-center"
                  >
                    ...
                    <div>
                      ...
                      <h3 className="mt-2 text-base font-black text-text-primary">
                        {title}
                      </h3>
                      <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-text-secondary">
                        <span>
                          Nông hộ: <strong>{farmer}</strong>
                        </span>
                        <span>
                          Cây trồng: <strong>{crop}</strong>
                        </span>
                        <span>
                          AI tự tin:{" "}
                    ...
                    <div className="flex gap-2 md:flex-col">
                      <button className="rounded-lg bg-primary px-4 py-2 text-xs font-black text-white">
                        Mở ca
                      </button>
                      <button className="rounded-lg border border-border-subtle bg-white px-4 py-2 text-xs font-black text-text-secondary">
                        Gán kỹ sư
                      </button>
                    </div>
                  </article>
          ...
          <section className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm xl:col-span-2">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-text-muted">
                  Theo dõi số lượng, tốc độ bán, hạn dùng và đề xuất nhập bổ
                  sung.
                </p>
              </div>
              <div className="flex gap-2">
                <button className="rounded-lg border border-border-subtle px-4 py-2 text-sm font-black text-text-secondary">
                  Nhập kho
                </button>
                <button className="rounded-lg bg-primary px-4 py-2 text-sm font-black text-white">
                  Xuất kiểm kê
          ...
          <section className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm xl:col-span-2">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-text-muted">
                  Theo dõi xuất kho, giao vận, thanh toán và chứng từ hóa đơn.
                </p>
              </div>
              <button className="rounded-lg bg-primary px-4 py-2 text-sm font-black text-white">
                Tạo đơn nhanh
              ...
              {[
                ...
                >
                  <div className="flex flex-col gap-3 md:flex-row md:items-center">
                    <div className="min-w-[92px] font-black text-primary">
                      {code}
                    </div>
                    ...
                    <StatusPill
                      tone={
                        index === 1 ? "amber" : index === 0 ? "blue" : "green"
                      }
                    >
                      {status}
                    </StatusPill>
                    ...
                    <button className="rounded-lg border border-border-subtle bg-white px-4 py-2 text-xs font-black text-primary">
                      Chi tiết
                    </button>
                  </div>
                </article>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
          <section className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm xl:col-span-2">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-black text-text-primary">
                  Sổ nợ nông hộ liên kết
                </h2>
                <p className="mt-1 text-xs text-text-muted">
                  Quản lý hạn mức, dư nợ, lịch nhắc thanh toán và bảo lãnh HTX.
                </p>
              </div>
              <button className="rounded-lg bg-primary px-4 py-2 text-sm font-black text-white">
                Tạo phiếu ghi nợ
          ...
          <section className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 space-y-3">
              {[
                ...
                <button
                  key={title}
                  className="w-full rounded-lg border border-border-subtle bg-surface-subtle p-4 text-left hover:border-primary"
                >
                  <div className="font-black text-text-primary">{title}</div>
                  <div className="mt-1 text-xs text-text-muted">{desc}</div>
                </button>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
          <section className="rounded-xl border border-border-subtle bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 rounded-2xl border border-primary bg-primary-light p-5 text-center">
              ...
              <div className="mt-4 font-black text-primary">
                VCB · AGRISAGE LÂM ĐỒNG
              </div>
              <div className="mt-1 text-xs text-text-muted">
                STK 190068289999 · Nội dung: mã đơn
              </div>
            </div>
            <div className="mt-4 flex gap-2">
              <button className="flex-1 rounded-lg bg-primary px-4 py-2 text-sm font-black text-white">
                Tải QR
              </button>
              <button className="flex-1 rounded-lg border border-border-subtle px-4 py-2 text-sm font-black text-primary">
                In để quầy
              </button>
            </div>
          </section>
          ...
          <section className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm xl:col-span-2">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-black text-text-primary">
                  Đối soát giao dịch gần đây
                </h2>
                <p className="mt-1 text-xs text-text-muted">
                  Tự khớp tiền về với đơn hàng, sổ nợ hoặc thanh toán một phần.
                </p>
              </div>
              <button className="rounded-lg bg-primary px-4 py-2 text-sm font-black text-white">
                Xuất sao kê
            ...
            <div className="space-y-3">
              {[
                ...
                <div
                  key={`${time}-${content}`}
                  className="flex flex-col gap-3 rounded-lg border border-border-subtle bg-surface-subtle p-4 md:flex-row md:items-center"
                >
                  <div className="font-black text-text-muted md:w-16">
                    {time}
                  </div>
                  <div className="flex-1">
                    <div className="font-black text-text-primary">
                  ...
                  <StatusPill tone={tone as "green" | "amber" | "blue"}>
                    {tone === "amber" ? "Chờ xử lý" : "Đã khớp"}
                  </StatusPill>
                </div>
              ))}
            </div>
        ...
        <section className="overflow-hidden rounded-2xl border border-border-subtle bg-white shadow-sm">
          <div className="bg-primary px-6 py-8 text-white">
            <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
              ...
              <button className="rounded-lg bg-white px-4 py-2 text-sm font-black text-primary">
                Cập nhật hồ sơ
              </button>
            </div>
          </div>
          ...
          <section className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 space-y-3">
              {[
                ...
                <div
                  key={name}
                  className="flex items-center gap-3 rounded-lg bg-surface-subtle p-3"
                >
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-primary-light font-black text-primary">
                    {name.charAt(0)}
                  </div>
                  <div className="flex-1">
                  ...
                  <StatusPill tone={status === "Đang giao" ? "blue" : "green"}>
                    {status}
                  </StatusPill>
                </div>
              ))}
  ...
  return (
    <>
      <section className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-5">
        <MiniMetric label="Tổng doanh thu (T10)" value="482.65tr" icon="trending_up" />
        <MiniMetric label="Lợi nhuận gộp" value="115.84tr" icon="payments" />
        <MiniMetric label="Dòng tiền thực thu" value="346.25tr" icon="account_balance" tone="blue" />
        <MiniMetric label="Gối đầu vụ mùa" value="136.4tr" icon="cycle" tone="amber" />
        <MiniMetric label="Giá trị đơn TB" value="1.485tr" icon="receipt" />
      </section>
      ...
      <section className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-3">
        <div className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm xl:col-span-2">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-black text-text-primary">
                Lịch vận hành cần xử lý hôm nay
              </h2>
              <p className="mt-1 text-xs text-text-muted">
                Các đầu việc ảnh hưởng trực tiếp đến đơn hàng, kho và sổ nợ mùa
                vụ.
              </p>
            </div>
            <button className="rounded-lg border border-border-subtle px-4 py-2 text-sm font-black text-primary">
              Xem lịch tuần
            </button>
          </div>
          <div className="space-y-3">
            {[
              ...
              [
                "10:00",
                "Duyệt 4 ca AI thán thư sầu riêng",
                "SLA 30 phút",
                "Kỹ thuật",
              ],
              [
                "14:00",
                "Nhắc nợ 5 nông hộ đến hạn",
                "48.500.000 đ",
                "AgriCredit",
              ],
