...
function CategoryBadge({ category }: { category: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-semibold ${styles[category] || "bg-surface-secondary text-text-secondary"}`}
    >
...
export default function Knowledge() {
  ...
  const visibleArticles = filtered.filter(
    (a) => activeCat || search || a.id !== featured.id,
  )
      ...
      <div className="bg-gradient-to-r from-primary-dark to-primary text-white py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold mb-4">
            Kiến thức canh tác & Cảnh báo dịch hại
          </h1>
          <p className="text-emerald-100/90 text-sm max-w-xl">
            Cập nhật thông tin nông nghiệp mới nhất, kỹ thuật canh tác tiên tiến
            và cảnh báo dịch bệnh vùng từ đội ngũ kỹ sư AgriSage.
          </p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 bg-white rounded-2xl border border-border-subtle overflow-hidden shadow-sm hover:shadow-card transition-all">
              ...
              <div className="lg:col-span-7 p-6 sm:p-8 flex flex-col justify-between">
                <div>
                  ...
                  <h2 className="text-xl sm:text-2xl font-bold text-text-primary group-hover:text-primary transition-colors leading-snug mb-3">
                    {featured.title}
                  </h2>
          ...
          <>
            <div className="mb-4 text-sm text-text-muted">
              ...
              {visibleArticles.length > PAGE_SIZE && (
                <span className="ml-1">
                  - trang{" "}
                  <strong className="text-text-primary">{safePage}</strong>/
                  {totalPages}
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {pagedArticles.map((a) => (
                <article
                  key={a.id}
                  className="bg-white rounded-xl border border-border-subtle overflow-hidden hover:shadow-card transition-all group flex flex-col"
                >
                  <Link
                    to={`/kien-thuc/${a.slug}`}
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {a.tags.slice(0, 3).map((tag) => (
                        <span
                          key={tag}
                          className="px-2 py-0.5 bg-surface-subtle text-text-muted text-[11px] rounded"
                        >
                          {tag}
                        </span>
                      ))}
