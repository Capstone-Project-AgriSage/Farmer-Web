  ...
  const reviews = [
    {
      ...
      rating: 5,
      comment:
        "Sản phẩm rất hiệu quả, sử dụng 1 tuần là thấy cây hồi phục rõ rệt. Sẽ mua lại!",
    },
    {
      name: "Trần Thị Hoa",
      date: "08/10/2024",
      rating: 5,
      comment:
        "Hàng chính hãng, đóng gói cẩn thận. Giao hàng nhanh, còn kèm hướng dẫn sử dụng tiếng Việt.",
    },
    {
      name: "Lê Minh Đức",
      date: "05/10/2024",
      rating: 4,
      comment:
        "Giá hợp lý, hiệu quả tốt với bệnh thán thư sầu riêng. Phun 3 lần là sạch bệnh hoàn toàn.",
    },
  ]
      ...
      <div className="bg-white border-b border-border-subtle">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex items-center gap-2 text-xs text-text-muted">
            ...
            <span className="text-text-primary font-medium line-clamp-1 max-w-xs">
              {product.name}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5">
            <div className="bg-white rounded-2xl border border-border-subtle p-8 flex items-center justify-center aspect-square shadow-sm relative">
              <span className="absolute top-4 left-4 px-2.5 py-1 bg-primary text-white text-xs font-bold rounded">
                {product.brand}
              </span>
              {discount > 0 && (
                <span className="absolute top-4 right-4 px-2.5 py-1 bg-red-500 text-white text-xs font-bold rounded">
                  -{discount}%
                </span>
              )}
              <img
          ...
          <div className="lg:col-span-7 space-y-5">
            <div>
              ...
              <h1 className="text-xl sm:text-2xl font-bold text-text-primary leading-snug mb-3">
                {product.name}
              </h1>
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <span
                      key={i}
                      className={`material-symbols-outlined fill text-[16px] ${
            ...
            <div className="bg-surface-subtle rounded-xl p-4 border border-border-subtle">
              <div className="flex items-end gap-3">
                ...
                {product.originalPrice && (
                  <div className="text-base text-text-muted line-through pb-0.5">
            ...
            <div className="space-y-2 text-sm">
              {[
                ...
                { label: "Quy cách đóng gói", value: product.packaging },
                {
                  ...
                  green: product.inStock,
                },
              ].map((row) => (
                <div key={row.label} className="flex gap-3">
                  <span className="text-text-muted w-40 flex-shrink-0">
                    {row.label}:
                  </span>
                  <span
                    className={`font-medium ${
                      row.green ? "text-status-success" : "text-text-primary"
                    }`}
                  >
                    {row.value}
                  </span>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {product.crops.map((c) => (
                    <span
                      key={c}
                      className="px-2.5 py-1 bg-primary-light text-primary text-xs font-medium rounded-full"
                    >
                      {c}
                    </span>
                  ))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {product.diseases.map((d) => (
                    <span
                      key={d}
                      className="px-2.5 py-1 bg-status-warning-surface text-status-warning text-xs font-medium rounded-full"
                    >
                      {d}
                    </span>
                  ))}
            ...
            <div className="flex items-center gap-4 pt-2">
              <div className="flex items-center border border-border-subtle rounded-lg overflow-hidden bg-white">
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center text-text-secondary hover:bg-surface-subtle transition-colors"
                >
              ...
              <button
                onClick={handleAdd}
                className={`flex-1 py-3 px-6 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-sm ${
                  added
            ...
            <div className="grid grid-cols-3 gap-3 pt-1">
              {[
                ...
                <div
                  key={b.label}
                  className="flex flex-col items-center gap-1.5 text-center p-3 bg-surface-subtle rounded-lg border border-border-subtle"
                >
            ...
            {tab === "usage" && (
              <div className="bg-primary-light rounded-xl p-5">
                <div className="flex items-start gap-3">
                  <span className="material-symbols-outlined text-[24px] text-primary mt-0.5">
                    science
                  </span>
                  <div>
                    ...
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div className="flex items-center gap-0.5 ml-auto">
                        {[1, 2, 3, 4, 5].map((i) => (
                          <span
                            key={i}
                            className={`material-symbols-outlined fill text-[14px] ${
                              i <= r.rating
                                ? "text-yellow-400"
                                : "text-border-strong"
                            }`}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {related.map((p) => (
                <Link
                  key={p.id}
                  to={`/san-pham/${p.slug}`}
                  className="bg-white rounded-xl border border-border-subtle hover:border-primary hover:shadow-sm transition-all overflow-hidden group"
                >
                  ...
                  <div className="p-3 border-t border-border-subtle">
                    <div className="text-xs font-bold text-text-primary group-hover:text-primary line-clamp-2">
                      {p.name}
                    </div>
