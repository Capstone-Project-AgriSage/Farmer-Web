...
function CategoryBadge({ category }: { category: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-semibold ${styles[category] || "bg-surface-secondary text-text-secondary"}`}
    >
      <span className="material-symbols-outlined text-[14px]">
        {icons[category] || "article"}
      </span>
      {category}
    </span>
  )
  ...
  return lines.map((line, i) => {
    ...
    if (trimmed.startsWith("- ")) {
      const html = trimmed
        ...
        .replace(
          /`(.+?)`/g,
          "<code class='bg-surface-secondary px-1 py-0.5 rounded text-xs font-mono'>$1</code>",
        )
      return (
        <li
          key={i}
          className="text-sm text-text-secondary leading-relaxed mb-1 ml-4 list-disc"
    ...
    if (/^\d+\./.test(trimmed)) {
      ...
      return (
        <li
          key={i}
          className="text-sm text-text-secondary leading-relaxed mb-1 ml-4 list-decimal"
      ...
      <div className="bg-white border-b border-border-subtle">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex items-center gap-2 text-xs text-text-muted flex-wrap">
            ...
            <Link to="/kien-thuc" className="hover:text-primary">
              Kiến thức
            </Link>
            <span className="material-symbols-outlined text-[14px]">
              chevron_right
          ...
          <div className="lg:col-span-8">
            <article className="bg-white rounded-2xl border border-border-subtle shadow-sm overflow-hidden">
              ...
              <div className="p-6 sm:p-8">
                ...
                <h1 className="text-xl sm:text-2xl font-bold text-text-primary leading-snug mb-4">
                  {article.title}
                </h1>
                ...
                <div className="flex items-center gap-3 pb-5 mb-5 border-b border-border-subtle">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-text-muted">
                      Chuyên gia Nông nghiệp AgriSage
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-8 pt-5 border-t border-border-subtle">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {article.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-3 py-1 bg-surface-subtle border border-border-subtle text-xs text-text-secondary rounded-full hover:border-primary hover:text-primary transition-colors cursor-pointer"
                      >
                        #{tag}
                      </span>
                    ))}
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                      <div className="font-bold mb-1">
                        Cần tư vấn thêm về bệnh cây trồng?
                      </div>
                      <div className="text-xs text-emerald-100/90">
                        Thử ngay Bác sĩ AI miễn phí hoặc gọi kỹ sư tư vấn
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <Link
                        to="/bac-si-ai"
                        className="px-4 py-2 bg-white text-primary-dark rounded-lg text-xs font-bold hover:bg-emerald-50 transition-colors whitespace-nowrap"
                      >
                        Bác sĩ AI
                      </Link>
                      <a
                        href="tel:19006828"
                        className="px-4 py-2 bg-white/15 border border-white/20 text-white rounded-lg text-xs font-semibold hover:bg-white/25 transition-colors whitespace-nowrap"
                      >
                        1900 6828
                      </a>
                    </div>
          ...
          <div className="lg:col-span-4 space-y-5">
              ...
              <div className="space-y-4">
                {others.map((a) => (
                  ...
                  >
                    ...
                    />
                    <div>
                      <h4 className="text-xs font-bold text-text-primary group-hover:text-primary transition-colors line-clamp-2 leading-snug">
                        {a.title}
                      </h4>
                      <div className="text-[11px] text-text-muted mt-1">
                        {a.date} · {a.readTime} phút
                      </div>
                    </div>
                  </Link>
                ))}
            ...
            <div className="bg-gradient-to-br from-primary-dark to-primary rounded-2xl p-5 text-white">
              ...
              <p className="text-xs text-emerald-100/90 mb-4 leading-relaxed">
                Chụp ảnh cây bị bệnh và nhận chẩn đoán AI miễn phí trong vài
                giây.
              </p>
              <Link
                to="/bac-si-ai"
                className="block w-full py-2 bg-white text-primary-dark text-xs font-bold rounded-lg text-center hover:bg-emerald-50 transition-colors"
              >
                Thử ngay - Miễn phí
              </Link>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-border-subtle shadow-sm p-5">
              <h3 className="font-bold text-text-primary text-sm mb-3">
                Đường dây tư vấn kỹ thuật
              </h3>
              <a
                href="tel:19006828"
                className="flex items-center gap-3 p-3 bg-primary-light rounded-xl"
              >
