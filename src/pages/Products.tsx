      ...
      <div className="bg-gradient-to-r from-primary-dark to-primary text-white py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <form onSubmit={handleSearch} className="flex gap-2 max-w-xl">
            ...
            <button
              type="submit"
              className="px-5 py-2 bg-white text-primary-dark font-semibold text-sm rounded-lg hover:bg-emerald-50 transition-colors"
            >
              Tìm
            </button>
          </form>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-4 text-sm text-text-muted">
          {q && (
            <span>
              ...
              <strong className="text-text-primary">{q}</strong>":{" "}
            </span>
          )}
          <strong className="text-text-primary">{filtered.length}</strong> sản
          phẩm
          {filtered.length > PAGE_SIZE && (
            <span className="ml-1">
              - trang <strong className="text-text-primary">{safePage}</strong>/
              {totalPages}
            </span>
          )}
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <div className="text-text-secondary mt-4 text-lg font-semibold">
            ...
            <button
              onClick={() => setParams({})}
              className="mt-4 px-5 py-2 bg-primary text-white rounded-lg text-sm font-semibold hover:bg-primary-hover transition-colors"
            >
              Xem tất cả
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {pagedProducts.map((p) => (
              <div
                key={p.id}
                className="bg-white rounded-xl border border-border-subtle hover:border-primary hover:shadow-card transition-all duration-300 overflow-hidden flex flex-col group"
              >
                <Link
                  to={`/san-pham/${p.slug}`}
                  className="relative p-4 bg-white flex items-center justify-center h-48 overflow-hidden"
                >
                  ...
                  <span
                    className={`absolute bottom-2 right-2 px-2 py-0.5 text-[10px] font-semibold rounded flex items-center gap-1 z-10 ${
                      p.inStock
                  ...
                  >
                    <span
                      className={`w-1.5 h-1.5 rounded-full ${
                        p.inStock ? "bg-status-success" : "bg-status-error"
                      }`}
                    ></span>
                ...
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  <Link to={`/san-pham/${p.slug}`}>
                    <h3 className="text-sm font-bold text-text-primary group-hover:text-primary transition-colors line-clamp-2 leading-snug mb-1">
                      {p.name}
                    </h3>
                  </Link>
