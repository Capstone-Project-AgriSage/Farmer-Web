...
function CheckoutSteps({ step }: { step: CheckoutStep }) {
  ...
  const circleClass = (active: boolean) =>
    `flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-black ${
      active ? "bg-primary text-white" : "bg-surface-secondary text-text-muted"
    }`
    ...
    <section className="overflow-x-auto rounded-lg border border-border-subtle bg-white shadow-sm">
      ...
      <div className="min-w-[760px] divide-y divide-border-subtle">
        {items.map((item) => (
          <div
            key={item.id}
            className="grid grid-cols-[1fr_105px_105px_120px_24px] items-center gap-4 px-4 py-3"
          >
            <div className="flex min-w-0 gap-3">
              <Link
                to={`/san-pham/${item.id}`}
                className="flex h-16 w-16 shrink-0 items-center justify-center rounded-lg border border-border-subtle bg-white p-1.5"
              >
                <img
                  src={item.image}
                  alt={item.name}
              ...
              <div className="min-w-0">
                ...
                <Link
                  to={`/san-pham/${item.id}`}
                  className="line-clamp-2 text-sm font-black leading-snug text-primary hover:text-primary-hover"
                >
                  {item.name}
                </Link>
                <div className="mt-1 text-xs text-text-muted">
                  Quy cách: {item.unit} | Hàng chính hãng
                </div>
              </div>
            </div>
            <div className="text-right text-sm font-black text-text-primary">
            ...
            <div className="flex justify-center">
              <div className="flex h-7 items-center overflow-hidden rounded-full border border-border-subtle bg-white">
                <button
                  ...
                  className="flex h-full w-8 items-center justify-center text-sm font-black text-text-secondary hover:bg-surface-subtle"
                >
                  -
                </button>
            ...
            <button
              ...
              className="justify-self-end text-text-muted hover:text-status-error"
    ...
    <section className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-base font-black">
          {compact ? "Đơn hàng của bạn" : "Tóm tắt đơn hàng"}
        </h2>
        <span className="rounded-full bg-primary-light px-2.5 py-1 text-xs font-bold text-text-muted">
          ({count} sản phẩm)
        </span>
      </div>
          ...
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-3 py-3">
              <img
                src={item.image}
                alt={item.name}
                className="h-12 w-12 rounded-lg border border-border-subtle object-contain p-1"
              />
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-text-muted">
                  Số lượng: x{item.quantity} {item.unit.toLowerCase()}
                </div>
              </div>
      ...
      <div className="space-y-3 text-sm">
        ...
        <div className="flex justify-between gap-3">
          <span className="text-text-secondary">
            Phí vận chuyển xe tải tận vườn:
          </span>
          <span className="font-black text-primary">Miễn phí</span>
        </div>
        <div className="flex justify-between gap-3">
          <span className="text-text-secondary">
            Thuế VAT (Hóa đơn đỏ điện tử):
          </span>
      ...
      <div className="mt-5 border-t border-border-subtle pt-4">
        ...
        <div className="mt-1 text-right text-xs font-bold text-primary">
          Tiết kiệm 50.000 đ cho mùa vụ này
        </div>
      </div>
  ...
  return (
    <section className="rounded-xl border border-border-subtle bg-white p-6 shadow-sm">
      <div className="mb-5 flex items-center gap-3 border-b border-border-subtle pb-4">
        <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary-light text-sm font-black text-primary">
          {number}
        </span>
...
function CheckoutPage({
  ...
  setShipping: React.Dispatch<React.SetStateAction<{
    name: string
    phone: string
    address: string
    note: string
          ...
          <CheckoutCard number={1} title="Thông tin người nhận & Địa chỉ vườn">
            <div className="mb-5 grid gap-3 sm:grid-cols-2">
              <button className="rounded-lg border-2 border-primary bg-primary-light/40 p-4 text-left">
                ...
                <div className="mt-1 text-xs text-text-muted">
                  Xe tải hoặc bán tải đưa vào tận nơi
                </div>
              </button>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="text-xs font-black">
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-lg border border-border-subtle bg-[#f8fcf6] px-3 py-2.5 text-sm font-medium outline-none focus:border-primary"
              ...
              {["Tỉnh / Thành phố", "Huyện / Thị xã", "Xã / Thị trấn"].map(
                (label, index) => (
                  <label key={label} className="text-xs font-black">
                    ...
                    <select className="mt-1 w-full rounded-lg border border-border-subtle bg-[#f8fcf6] px-3 py-2.5 text-sm font-medium outline-none focus:border-primary">
                      <option>
                        {["Lâm Đồng", "Huyện Di Linh", "Xã Đinh Lạc"][index]}
                      </option>
                    </select>
                  </label>
                ),
              ...
              <label className="text-xs font-black sm:col-span-2">
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-lg border border-border-subtle bg-[#f8fcf6] px-3 py-2.5 text-sm font-medium outline-none focus:border-primary"
                  placeholder="Số 45 Thôn Tân Lạc..."
                />
              </label>
              <label className="text-xs font-black sm:col-span-2">
                ...
                <textarea
                  ...
                  rows={3}
                  className="mt-1 w-full rounded-lg border border-border-subtle bg-[#f8fcf6] px-3 py-2.5 text-sm font-medium outline-none focus:border-primary"
                  placeholder="Đường bê tông xe tải vào được..."
                />
              </label>
            </div>
          </CheckoutCard>
              ...
              <div className="rounded-lg border-2 border-primary bg-primary-light/30 p-4">
                <div className="flex items-center justify-between text-sm font-black">
                  <span className="flex items-center gap-2">
                    ...
                    Xe tải giao tận vườn AgriExpress
                  </span>
                  <span className="text-primary">0 đ</span>
                </div>
                <p className="mt-1 pl-7 text-xs text-text-muted">
                  Chuyên chở phân bón, bao nặng 50kg, hỗ trợ bốc xếp xuống tận
                  kho vườn.
                </p>
              </div>
            ...
            <div className="space-y-3">
                  ...
                  <div className="mt-4 grid gap-4 rounded-lg bg-white/70 p-4 sm:grid-cols-[150px_1fr]">
                    <div className="rounded-lg border border-border-subtle bg-white p-3 text-center">
                      <div className="flex h-28 items-center justify-center rounded-md bg-primary-light">
                        <span className="material-symbols-outlined text-[58px] text-primary">
                          qr_code_2
                        </span>
                      </div>
              ...
              {[
                ...
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-lg border p-4 text-left text-sm font-black ${
                    payMethod === key
                      ? "border-primary bg-primary-light/30"
                      : "border-border-subtle"
                  }`}
            ...
            <OrderSummary
              ...
              button={
                <button
                  ...
                  disabled={processing}
                  className="mt-6 flex h-14 w-full items-center justify-center gap-2 rounded-xl bg-primary px-5 text-base font-black uppercase text-white shadow-sm hover:bg-primary-hover disabled:opacity-70"
                >
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <section className="relative min-h-[430px] overflow-hidden rounded-2xl border border-border-subtle bg-white px-5 py-8 text-center shadow-sm">
          ...
          <div className="relative mx-auto max-w-[680px]">
            ...
            <div className="mb-2 inline-flex rounded-full bg-primary-light px-4 py-1 text-[11px] font-black uppercase tracking-wide text-primary">
              Xác nhận thành công - đã ghi nhận vào kho Di Linh
            </div>
            <h1 className="text-[27px] font-black leading-tight text-text-primary">
              Cảm ơn Bác nông dân Nguyễn Văn Hùng!
            </h1>
            <p className="mx-auto mt-2 max-w-[620px] text-sm leading-relaxed text-text-secondary">
              Đơn hàng <strong className="text-primary">#AGR-8842</strong> của
              Bác đã được tiếp nhận thành công trên hệ sinh thái AgriSage. Bộ
              phận kho vận đang xuất bao bì và chuẩn bị điều xe tải giao vật tư
              đến tận vườn.
            </p>
            ...
            <div className="mx-auto mt-5 grid max-w-[620px] gap-3 sm:grid-cols-4">
              {[
                ...
                <div
                  key={label}
                  className="rounded-lg bg-[#f2f8f1] p-3.5 text-left"
                >
            ...
            <Link
              to="/san-pham"
              className="mt-4 inline-flex h-10 items-center gap-2 rounded-lg border border-primary bg-white px-5 text-sm font-black text-primary hover:bg-primary-light"
            >
              <span className="material-symbols-outlined text-[17px]">
                storefront
              </span>
...
export default function Cart() {
  ...
  const [shipping, setShipping] = useState({
    ...
    phone: "0918 234 567",
    address: "Số 45 Thôn Tân Lạc, gần dốc ngã ba vườn sầu riêng Chú Năm",
  ...
  const [completedOrder, setCompletedOrder] = useState<OrderSnapshot | null>(
    null,
  )
    ...
    <div className="min-h-screen bg-[#f3faf2] text-text-primary">
      ...
      {items.length === 0 ? (
        <div className="mx-auto max-w-7xl px-4 py-16 text-center sm:px-6 lg:px-8">
          <span className="material-symbols-outlined text-[72px] text-text-muted">
            shopping_cart
          </span>
          <h1 className="mt-4 text-2xl font-black">Giỏ hàng đang trống</h1>
          <p className="mt-2 text-sm text-text-muted">
            Hãy thêm sản phẩm vào giỏ để tiếp tục đặt hàng.
          </p>
          <Link
            to="/san-pham"
            className="mt-6 inline-flex items-center gap-2 rounded-lg bg-primary px-5 py-3 text-sm font-black text-white hover:bg-primary-hover"
          >
        ...
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <div className="mb-5 flex items-center justify-between gap-4">
            <h1 className="flex items-center gap-2 text-xl font-black">
              <span className="material-symbols-outlined text-[23px] text-primary">
                shopping_basket
              </span>
              Giỏ hàng của bạn
              <span className="rounded-full bg-primary-light px-2.5 py-1 text-xs font-bold text-text-muted">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
            <div className="space-y-5 lg:col-span-8">
              ...
              />
              <section className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm">
                <h2 className="mb-3 text-sm font-black uppercase text-text-muted">
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  ...
                  <button className="rounded-lg bg-primary px-5 py-3 text-sm font-black text-white hover:bg-primary-hover">
                    Áp dụng
                  </button>
                  <Link
                    to="/san-pham"
                    className="inline-flex items-center justify-center gap-1 rounded-lg px-4 py-3 text-sm font-black text-primary hover:bg-primary-light"
                  >
                    <span className="material-symbols-outlined text-[17px]">
                      arrow_back
                    </span>
              ...
              <OrderSummary
                ...
                button={
                  <button
                    ...
                    className="mt-5 flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-primary px-5 text-xs font-black uppercase text-white hover:bg-primary-hover"
                  >
              ...
              <section className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-text-muted">
                      Miễn phí đơn thuốc bảo vệ thực vật trước khi giao
                    </div>
                  </div>
                  <a
                    href="tel:19006828"
